...
sequenceDiagram
  autonumber
  actor Author as 記事執筆者
  actor Owner as レビュワー
  participant AmbrRepo as ambr-zenn-contents（GitHub）
  participant GA as GitHub Actions
  participant AuthorRepo as 執筆者のzenn-contents（GitHub）
  participant Zenn as Zenn

  Author->>AmbrRepo: 記事追加/更新のPRを作成
  Owner->>AmbrRepo: PRをレビューしてマージ
  AmbrRepo-->>GA: pull_request closedをトリガー
  GA->>AuthorRepo: 記事内容を反映したPRを自動作成
  Author->>AuthorRepo: PRをマージ
  AuthorRepo->>Zenn: Zennで記事がドラフト状態で公開される
  Author->>AmbrRepo: 記事の公開フラグ(published)をtrueにするPRを作成
  AmbrRepo-->>GA: pull_request closedをトリガー
  GA->>AuthorRepo: 記事内容を反映したPRを自動作成
  Author->>AuthorRepo: 公開したいタイミングでPRをマージ
  AuthorRepo->>Zenn: Zennで記事が公開される
```
